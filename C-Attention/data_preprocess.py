...
def get_dt():
    # file_name = os.path.join(os.path.join(Path(os.getcwd()).parent, "data"), "final_full_interview_features.csv")
    file_name = os.path.join(os.path.join(Path(os.getcwd()).parent, "data"), "final_full_interview_features_and_other.csv")
    return pd.read_csv(file_name)

def get_embed_dt():
    return np.load(os.path.join("embedding_and_handcreafted_features", "embed.npy"))

def get_input_data():
    print("########### getting the inputs started ############")
    ...
    data = data.unsqueeze(1).type(torch.FloatTensor)#data[:,newaxis, :]
    df_train = data[:1163:]
    ...
    print("type:", type(df_train), " ", type(y_train))
    ...
    print("shape:", "\n", "x_embed_train", x_pos_train.size(), "x_embed_valid", x_pos_valid.size(), "y_train",
          y_train.size(), "y_valid", y_valid.size())
