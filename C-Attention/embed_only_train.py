...
def valid_epoch(model, validation_data, loss_func):
    ...
    for step, (batch_x, batch_y) in enumerate(validation_data):
        with torch.no_grad():
            predict_val = model(batch_x).squeeze(dim=1)
...
def train(EPOCH, len_sentences, training_data, validation_data):

    model = att_model.make_model_embed(D_w=len_sentences, N=6)
