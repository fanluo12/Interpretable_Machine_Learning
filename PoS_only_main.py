########################### import ###############################
from PoS_only_train import train
...
def main():
    ...
    for i in range(len(x_pos_train)):
        train_data.append([x_pos_train[i], y_train[i]])
    val_data = []
    for i in range(len(x_pos_valid)):
        val_data.append([x_pos_valid[i], y_valid[i]])
    ...
    best_model = train( EPOCH,  train_data, val_data)
    pre_np = test(best_model, x_pos_test)
